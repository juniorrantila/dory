...
constexpr ErrorOr<Core::JsonValue> parse(StringView, View<Token>,
    Vector<Core::JsonObject>&, Vector<Core::JsonArray>&);

}

namespace Core {

constexpr Json::Json(JsonValue root, Vector<JsonObject>&& objects,
...
ErrorOr<Json> Json::create_from(StringView source)
{
    ...
    auto objects = TRY(Vector<Core::JsonObject>::create());
    auto arrays = TRY(Vector<Core::JsonArray>::create());
    return Json {
...
ErrorOr<SingleValue> parse_single_value(StringView source,
    View<Token> tokens, Vector<Core::JsonObject>& objects,
    Vector<Core::JsonArray>& arrays);
    ...
    Token token)
{
    ...
    return SingleValue {
        Core::JsonValue(source.sub_view(token.start + "\""sv.size,
            token.size - "\"\""sv.size)),
        1,
    };
}
...
ErrorOr<SingleValue> parse_object(StringView source,
    ...
    Vector<Core::JsonArray>& arrays)
{
    auto object = TRY(Core::JsonObject::create());
...
ErrorOr<SingleValue> parse_array(StringView source,
    ...
    Vector<Core::JsonArray>& arrays)
{
    auto array = TRY(Core::JsonArray::create());
...
{
    return SingleValue { Core::JsonValue(nullptr), 1 };
}

constexpr SingleValue parse_true()
{
    return SingleValue { Core::JsonValue(true), 1 };
}

constexpr SingleValue parse_false()
{
    return SingleValue { Core::JsonValue(false), 1 };
}
...
constexpr ErrorOr<Core::JsonValue> parse(StringView source,
    View<Token> tokens, Vector<Core::JsonObject>& objects,
    Vector<Core::JsonArray>& arrays)
{
    auto value
